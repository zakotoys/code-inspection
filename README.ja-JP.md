...
`artifacts/` の出力：`zakotoys-code-inspection-core-0.2.2.tgz`、`zakotoys-code-inspection-runtime-0.2.2.tgz`、`code-inspection-vscode-0.2.2.vsix`、`code-inspection-zed-0.2.2.wasm`。core/runtime/VS Code の各パッケージコマンドを個別に実行する場合は、事前にビルドが必要です。`package:zed` は自身で Cargo を実行します。
