...
更新：2026-09-12。Windows 本机的构建、打包和独立安装验证通过；GitHub 工作流已随 Draft PR #4 发布，组织 PR 与个人 fork 的 Linux 检查均已通过。
...
2026-09-12 首次发布提交 79db31c 的组织 PR 与个人 fork 工作流均成功，已在 Ubuntu 24.04 上执行 npm ci 和完整 verify 并上传 VSIX。组织运行编号 34672388916，fork 运行编号 34672364176；后续提交以对应检查结果为准。

## 7. 当前证据与限制

本地完整 verify 成功：25 个状态/协议测试、3 个开发宿主场景和 2 个独立安装场景。运行依赖已打入包，安装后真实错误与修复可见。VSIX 只读白名单归档检查会在文档更新后的最终包上再次执行。

尚未验证：队友机器、真实 Agent 客户端、远程/Web 环境和 VS Code 市场发布。当前是可安装开发预览，不是正式市场版本。
