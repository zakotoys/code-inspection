    ...
    async (scenario) => {
      ...
      try {
        ...
        vi.spyOn(InspectionEngine.prototype, "run").mockImplementation(async (request) => ({
          findings: [{
            id: "finding", inspector: "eslint", source: "eslint", severity: "error",
        ...
        const initial = await current.runInspection({ inspector: "eslint", trigger: "manual" });
        await expect.poll(async () => (await current.getRun({ runId: initial.run.runId })).snapshot.run.outcome)
          .toBe("completed");
        const queued = await current.runInspection({ inspector: "eslint", trigger: "save" });
